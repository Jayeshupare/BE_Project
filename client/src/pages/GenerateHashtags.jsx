import { Hash, Send, Copy, Check } from 'lucide-react'
...
const GenerateHashtags = () => {
    ...
    const onSubmitHandler = async (e) => {
        ...
        try {
            setLoading(true)
            const { data } = await axios.post('/api/ai/generate-hashtags', { text }, {
        ...
        <div className='h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700'>
            <form onSubmit={onSubmitHandler} className='w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200'>
                <div className='flex items-center gap-3'>
                    ...
                    <h1 className='text-xl font-semibold'>Smart Hashtag Generator</h1>
                </div>

                <p className='mt-6 text-sm font-medium'>Post Content / Topic</p>
                <textarea onChange={(e) => setText(e.target.value)} value={text} rows={4} className='w-full p-2 px-3 mt-2 outline-none text-sm rounded-md border border-gray-300 resize-none' placeholder='e.g., Just launched my new AI startup! Super excited to share it with the world.' required />
            ...
            <div className='w-full max-w-lg p-4 bg-white rounded-lg flex flex-col border border-gray-200 min-h-64 max-h-[600px]'>
                <div className='flex items-center justify-between'>
                    <div className='flex items-center gap-3'>
                        ...
                        <h1 className='text-xl font-semibold'>Generated Hashtags</h1>
                    </div>
                    {content && (
                ...
                {!content ? (
                    <div className='flex-1 flex justify-center items-center'>
                        <div className='text-sm flex flex-col items-center gap-5 text-gray-400'>
                            ...
                            <p>Enter post content to generate</p>
                        </div>
                    </div>
                ) : (
